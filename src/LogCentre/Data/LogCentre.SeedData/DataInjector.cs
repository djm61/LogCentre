        ...
        public static void Initialize(LogCentreDbContext context)
        {
            ...
            if (!context.Providers.Any())
            {
                providers = new[]
                {
                    ...
                    new Provider {Name = "Tomcat", Description = "Provider for Tomcat log files", Regex = "(?<Ddate>\\d{2}-\\w{3}-\\d{4} \\d{2}:\\d{2}:\\d{2}.\\d{3})\\s(?<Level>\\w+)\\s\\[(?<Thread>\\w+)\\]\\s(?<Source>\\w.*?)\\s(?<Line>.*)", LastUpdatedBy = "Seed"}
                };
